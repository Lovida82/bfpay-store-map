...
- **OCR 등록**: 이미지 업로드 → GPT-4o로 가맹점 정보 추출 (여러 개 동시 인식)
  - 가게 간판, 영수증, 제로페이맵 즐겨찾기 캡쳐 등 다양한 이미지 지원
...
- **비회원 안내**: 회원가입 없이도 지도 열람 가능, 회원가입 시 등록/검증/맛집 후기 공유 참여 가능
...
### 9. 관리자 페이지
- **접근 권한**: `jsw0504@ajupharm.co.kr` 계정만 접근 가능
- **대시보드**: 총 사용자, 가맹점, 검증, 후기 통계
- **사용자 관리**:
  - 전체 사용자 목록 조회
  - 신뢰 레벨 변경 (Lv.1~5)
- **가맹점 관리**:
...
11. 랜딩 페이지 비회원 안내 문구 추가
   ...
   - 회원가입 시 가맹점 등록, 결제 검증 참여 가능 안내
   - 별점/후기로 맛집 정보 공유 기능 안내
12. OCR 등록 설명에 제로페이맵 즐겨찾기 캡쳐 등록 안내 추가
13. 관리자 페이지 추가 (`/admin`)
   - 관리자 이메일: jsw0504@ajupharm.co.kr
   - 대시보드: 통계 현황 (사용자, 가맹점, 검증, 후기 수)
   - 사용자 관리: 목록 조회, 신뢰 레벨 변경
   - 가맹점 관리: 상태 변경 (대기/승인/거절/폐업), 삭제
   - 검증/후기 관리: 조회 및 삭제
   - AdminRoute 컴포넌트로 접근 제어
